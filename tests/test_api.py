...
from mkdocstrings_handlers import typescript
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("mkdocstrings_handlers.typescript")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.typescript._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.typescript"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocstrings_handlers.typescript`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in typescript.__all__ or not hasattr(typescript, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in typescript.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("mkdocstrings_handlers.typescript")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and _module_or_child("mkdocstrings_handlers.typescript", item.name)
        ):
