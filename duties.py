...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("mkdocstrings_handlers.typescript", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc mkdocstrings_handlers.typescript",
        nofail=True,
    )
...
def docs_deploy(ctx: Context) -> None:
    ...
    if "pawamoy-insiders/typescript" in origin:
        ctx.run("git remote add upstream git@github.com:mkdocstrings/typescript", silent=True, nofail=True)
        ctx.run(
